<?php
$nzshpcrt_gateways[$num]['name'] = 'GoEmerchant';
$nzshpcrt_gateways[$num]['internalname'] = 'goemerchant_payments';
$nzshpcrt_gateways[$num]['function'] = 'gateway_goemerchant';
$nzshpcrt_gateways[$num]['form'] = "form_goemerchant";
$nzshpcrt_gateways[$num]['submit_function'] = "submit_goemerchant";
$nzshpcrt_gateways[$num]['payment_type'] = "credit_card";

if(in_array('goemerchant_payments',(array)get_option('custom_gateway_options'))) {
...
function form_goemerchant(){
	$output ='<tr><td>Transaction Center ID</td><td><input name="goemerchant_tc_id" type="text" value="'.get_option('goemerchant_tc_id').'"/></td></tr>';
	$output.='<tr><td>Gateway ID</td><td><input name="goemerchant_gateway_id" type="text" value="'.get_option('goemerchant_gateway_id').'"/></td></tr>';
	$output.='<tr><td>Processor</td><td><input name="goemerchant_processor" type="text" value="'.get_option('goemerchant_processor').'"/></td></tr>';
	$output.='<tr><td>MID</td><td><input name="goemerchant_mid" type="text" value="'.get_option('goemerchant_mid').'"/></td></tr>';
	$output.='<tr><td>TID</td><td><input name="goemerchant_tid" type="text" value="'.get_option('goemerchant_tid').'"/></td></tr>';
	$output.='<tr><td>Transaction Type</td><td><input name="goemerchant_transaction_type" type="text" value="'.get_option('goemerchant_transaction_type').'"/><br/>(auth or sale)</td></tr>';

	return $output;
}

function submit_goemerchant(){
	update_option('goemerchant_tc_id',$_POST['goemerchant_tc_id']);
	update_option('goemerchant_gateway_id',$_POST['goemerchant_gateway_id']);
	update_option('goemerchant_processor',$_POST['goemerchant_processor']);
	update_option('goemerchant_mid',$_POST['goemerchant_mid']);
	update_option('goemerchant_tid',$_POST['goemerchant_tid']);
	update_option('goemerchant_transaction_type',$_POST['goemerchant_transaction_type']);
	return true;
}

function gateway_goemerchant($seperator, $sessionid){
	...
	$data['USER']	= get_option('goemerchant_tc_id');
	$data['PWD'] 	= get_option('goemerchant_gateway_id');
	$data['PROCESSOR'] 	= get_option('goemerchant_processor');
	$data['MID'] 	= get_option('goemerchant_mid');
	$data['TID'] 	= get_option('goemerchant_tid');
	$data['OPERATIONTYPE'] 	= get_option('goemerchant_transaction_type');
	...
	curl_setopt($connection,CURLOPT_URL,"https://secure.goemerchant.com/secure/gateway/xmlgateway.aspx");
